...
`;
  ...
  Renderer.prototype.renderInline = function(tokens, options, env) {
    let result = "";
    const rules3 = this.rules;
    for (let i = 0, len = tokens.length; i < len; i++) {
      const type = tokens[i].type;
      if (typeof rules3[type] !== "undefined") {
        result += rules3[type](tokens, i, options, env, this);
      } else {
  ...
  Renderer.prototype.render = function(tokens, options, env) {
    ...
    for (let i = 0, len = tokens.length; i < len; i++) {
      ...
      } else if (typeof rules3[type] !== "undefined") {
        result += rules3[type](tokens, i, options, env, this);
      } else {
  ...
  Core.prototype.process = function(state) {
    const rules3 = this.ruler.getRules("");
    for (let i = 0, l = rules3.length; i < l; i++) {
      rules3[i](state);
    }
  };
  ...
  ParserBlock.prototype.tokenize = function(state, startLine, endLine) {
    ...
    while (line < endLine) {
      ...
      let ok = false;
      for (let i = 0; i < len; i++) {
        ok = rules3[i](state, line, endLine, false);
        if (ok) {
  ...
  ParserInline.prototype.parse = function(str, md2, env, outTokens) {
    ...
    this.tokenize(state);
    const rules3 = this.ruler2.getRules("");
    const len = rules3.length;
    for (let i = 0; i < len; i++) {
      rules3[i](state);
    }
  ...
  TurndownService.prototype = {
    ...
    turndown: function(input) {
      ...
      return postProcess3.call(this, output);
    },
    /**
  ...
  var WRAPPED = /* @__PURE__ */ new Set(["fence", "code_block"]);
  var PLACEHOLDER = "\0";
  function createRenderer() {
    const md2 = new lib_default({ html: true, linkify: true, breaks: true }).use(import_markdown_it_task_lists.default);
    md2.core.ruler.push("gpe_lines", (state) => {
    ...
    for (const type of WRAPPED) {
      const orig = md2.renderer.rules[type];
      md2.renderer.rules[type] = (tokens, idx, options, env, self) => {
  ...
  function codeText(node) {
    ...
    for (const child of node.childNodes) {
      if (child.nodeType === 3) out += child.nodeValue;
        ...
        const isBlock2 = /^(DIV|P)$/.test(child.nodeName);
        out += (isBlock2 && out && !out.endsWith("\n") ? "\n" : "") + codeText(child);
      }
    }
    return out;
  }
  ...
  function createTurndown() {
    ...
    td2.use(gfm);
    const baseEscape = td2.escape.bind(td2);
    td2.escape = (text2) => baseEscape(text2.replace(/_/g, PLACEHOLDER)).replace(/\\\[/g, "[").replace(/\\\]/g, "]").replace(new RegExp(PLACEHOLDER, "g"), (ch, offset, s) => {
      const inWord = isWordChar(s[offset - 1]) && isWordChar(s[offset + 1]);
      return inWord ? "_" : "\\_";
    });
    td2.addRule("listItem", {
      filter: "li",
    ...
    td2.addRule("fencedCodeBlock", {
      ...
      replacement(content, node) {
        ...
        const code2 = codeText(node.firstChild).replace(/\n$/, "");
        return `
...
`;
    ...
    td2.addRule("sizedImage", {
      filter(node) {
        return node.nodeName === "IMG" && (node.hasAttribute("width") || node.hasAttribute("height") || node.hasAttribute("style"));
      },
      replacement(content, node) {
        return node.outerHTML;
      }
    });
    td2.keep(["kbd", "sup", "sub", "details", "summary", "video"]);
    return td2;
  }
  function blockToMarkdown(td2, elementOrHtml) {
    return td2.turndown(elementOrHtml).trim();
  }
  function spliceLines(src, start, end, replacement) {
  ...
  var CONTAINER_SEL = ".js-previewable-comment-form";
  var PREVIEW_PANEL_SEL = ".js-preview-panel, .preview-content";
  var BODY_SEL = 'textarea[name="pull_request[body]"], textarea[name="issue[body]"]';
  var BLOCK_SEL = `[${ATTR}]`;
  var COMMIT_DELAY = 250;
  function isPrPage() {
  ...
  `;
    ...
    const state = { src: "", blocks: [], timers: /* @__PURE__ */ new Map() };
    function render() {
      flushAll();
    ...
    function commit(block2) {
      state.timers.delete(block2);
      const markdown = blockToMarkdown(td, block2.el);
      const oldEnd = block2.end;
      const { src, delta } = spliceLines(state.src, block2.start, block2.end, markdown);
      if (delta === 0 && src === state.src) return;
      state.src = src;
      block2.end += delta;
      for (const b of state.blocks) {
        if (b !== block2 && b.start >= oldEnd) {
          b.start += delta;
          b.end += delta;
        }
      }
    ...
    function schedule(block2) {
      clearTimeout(state.timers.get(block2));
      state.timers.set(block2, setTimeout(() => commit(block2), COMMIT_DELAY));
    }
    function flushAll() {
      for (const [block2, timer] of state.timers) {
        clearTimeout(timer);
        commit(block2);
      }
      state.timers.clear();
    ...
    body.addEventListener("input", (e) => {
      const block2 = blockOf(e.target);
      if (block2) schedule(block2);
    });
    ...
    body.addEventListener("keydown", (e) => {
      ...
      const block2 = blockOf(e.target);
      if (!block2) return;
      if (/^H[1-6]$/.test(block2.el.tagName)) {
        e.preventDefault();
        return;
      }
